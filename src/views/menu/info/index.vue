  ...
  <div class="container">
    <div class="manage-title">
      <div class="header">
        ...
        >
          <el-tag :type="item.name ==='正常'?'success':'danger'">{{ item.name }}</el-tag>
        </span>
      </div>
      <div class="mid">
        <span style="margin-top: auto;margin-bottom: auto;font-size: 15px;">
          <span class="detail">
            ...
            <span>{{ infoData&&infoData.linkman?infoData.linkman:'-' }}</span>
          </span>
          <span class="detail">
            <span class="textFront">机构联系人电话:</span>
            <span>{{ infoData&&infoData.linktel?infoData.linktel:'-' }}</span>
          </span>
          <span class="detail">
            <span class="textFront">创建时间:</span>
            <span>{{ infoData&&infoData.createTime?infoData.createTime:'' }}</span>
          </span>
        </span>
        <img
          ...
          >
            <div v-if="item.name === '概况信息'">
              <h4 style="display: flex;justify-content: space-between">
                <span>基本信息</span>
                <span class="viewing" @click="findLicDetail(infoData.organizationCode)">查看证照详情 >>></span>
              </h4>
              <div class="normal">
                <div style="flex: 6;display: flex">
                  ...
                  <div class="right-col">
                    <p>
                      <span>{{ infoData&&infoData.organizationName?infoData.organizationName:'' }}</span>
                    </p>
                    <p>
                      <span>{{ infoData&&infoData.organizationType?infoData.organizationType:'' }}</span>
                    </p>
                  </div>
                </div>
                <div style="flex: 4;display: flex">
                  ...
                  <div class="right-col" style="flex:6; text-align: left">
                    <p>
                      <span>{{ infoData&&infoData.organizationCode?infoData.organizationCode:'-' }}</span>
                    </p>
                    <p>
                      <span>{{ infoData&&infoData.publicPrivateNature?natureMatter(infoData.publicPrivateNature):'-' }}</span>
                    </p>
                  </div>
                </div>
              </div>
              ...
              >
                <div class="detail1">
                  <div style="flex: 6;display: flex">
                    ...
                    <div class="right-col">
                      <!-- 法定代表人 -->
                      <p>
                        <span v-if="!isEdit">{{ infoData&&infoData.lawPerson?infoData.lawPerson:'-' }}</span>
                        <span v-else>
                      ...
                      <!-- 固定电话 -->
                      <p>
                        <span v-if="!isEdit">{{ infoData&&infoData.phoneNumber?infoData.phoneNumber:'-' }}</span>
                        <span v-else>
                      ...
                      <!-- 地址（县） -->
                      <p>
                        <span v-if="!isEdit">{{ infoData&&infoData.countyName ?infoData.countyName :'-' }}</span>
                        <span v-else>
                      ...
                      <!--联系人姓名  -->
                      <p>
                        <span v-if="!isEdit">{{ infoData&&infoData.linkman?infoData.linkman:'-' }}</span>
                        <span v-else>
                      ...
                      <!-- 电子邮箱 -->
                      <p>
                        <span v-if="!isEdit">{{ infoData&&infoData.faxNumber?infoData.faxNumber:'-' }}</span>
                        <span v-else>
                      ...
                      <!-- 业务范围 -->
                      <p v-if="!isEdit">
                        <span>{{ infoData&&infoData.businessScope?infoData.businessScope:'-' }}</span>
                      </p>
                      <el-input
                        type="textarea"
                        v-else
                  ...
                  <div style="flex: 4;display: flex">
                    ...
                    <div class="right-col" style="flex:6; text-align: left">
                      <!-- 登记管理机关 -->
                      <p>
                        <span v-if="!isEdit">{{ infoData&&infoData.registrationAuthority?infoData.registrationAuthority:'-' }}</span>
                        <span v-else>
                          <el-form-item prop="registrationAuthority">
                            <el-input v-model="detailData.registrationAuthority" />
                          </el-form-item>
                        </span>
                      </p>
                      <!-- 地址（市） -->
                      <p>
                        <span v-if="!isEdit">{{ infoData&&infoData.cityName ?infoData.cityName :'-' }}</span>
                        <span v-else>
                      ...
                      <!-- 详细地址 -->
                      <p>
                        <span v-if="!isEdit">{{ infoData&&infoData.addressInfo?infoData.addressInfo:'-' }}</span>
                        <span v-else>
                      ...
                      <!-- 机构联系人电话 -->
                      <p>
                        <span v-if="!isEdit">{{ infoData&&infoData.linktel?infoData.linktel:'-' }}</span>
                        <span v-else>
                      ...
                      <!-- 简介 -->
                      <p v-if="!isEdit">
                        <span>{{ infoData&&infoData.introduction?infoData.introduction:'-' }}</span>
                      </p>
                      <el-input
                        type="textarea"
                        v-else
                ...
                <div style="margin-top: 75px" v-show="cityCode !== 4&&!isOrganization">
              ...
              <table-template :table-data="tableData" :loading="loading">
                ...
                >
                  <template slot-scope="scope">
                    <span v-if="scope.row.name" @click="organizationCode(scope.row.identityDocumentNumber )">{{ scope.row.name }}</span>
                    <span v-else>- -</span>
                  </template>
                </el-table-column>
                <el-table-column
            ...
            <div v-else>
              <table-template :table-data="tableLogData" :loading="logLoading">
                ...
                >
                  <template slot-scope="scope">
                    <span v-if="scope.row.operTime">{{ parseTime(scope.row.operTime) }}</span>
                    <span v-else>--</span>
                  </template>
                </el-table-column>
                <el-table-column
...
export default {
  ...
  data(){
    return{
      ...
      leftColArr1:['法定代表人:','固定电话:','地址（县）','联系人姓名:','电子邮箱','业务范围:'],
      rightColArr:['统一社会信用代码:','机构性质:'],
      rightColArr1:['登记管理机关:','地址（市）','详细地址:','机构联系人电话:','简介:'],
      rules:{
        linktel:[
  ...
  async created(){//点其他进入
    ...
    if(this.$store.state.isOrganization){
      ...
      const {data,total} = await api.person.getSelfPerson(this.organizationId)
      this.total = total
      this.tableData = data
      this.loading = false
      const obj = {}
  ...
  watch:{
    isOrganization(val){
      if(!val) {//本机构的详情 isOrganization为false 证明此时点击导航进来的
        api.org.reqOrgInfo().then((response)=>{
          this.infoData = response.data
          this.detailData = this.infoData
        })
      }else{//isOrganization为true 由其他页面跳转过来的
